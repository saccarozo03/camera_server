...
_poll_task = None
_armed = True          # fail_reason==0 -> True, loi hop le -> trigger 1 lan roi False
_lock = asyncio.Lock() # chong trigger chong
	...
	while True:
		try:
			...
			if m:
				...
				# 1) ve OK -> mo co de trigger lan sau
				if fr == 0:
					if not _armed:
						log("[INFO] fail_reason back to 0 -> re-armed")
					_armed = True

				# 2) loi di chuyen hop le -> trigger 1 lan/dot
